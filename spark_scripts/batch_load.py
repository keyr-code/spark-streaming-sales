...
def create_spark_session():
    ...
    spark = (SparkSession.builder
        .appName("TheGoods-Data-Processor")
        .config("spark.jars.packages", "org.apache.hadoop:hadoop-aws:3.3.1,org.postgresql:postgresql:42.3.1")
...
def process_batch_data(spark):
    ...
    try:
        ...
        inventory_trans_df = read_from_minio(spark, bucket_name, "inventory_transactions.csv")
        ...
        if sales_df is not None:
            ...
            if sales_count > distinct_sales_count:
                logger.info(f"Found duplicate sale_id values: {sales_count} rows, {distinct_sales_count} unique sale_ids")
                ...
                sales_df = sales_df.groupBy('sale_id').agg(
                    ...
                    F.expr('approx_most_frequent(customer_id, 1)[0][0]').alias('customer_id'),
                    F.expr('approx_most_frequent(employee_id, 1)[0][0]').alias('employee_id'),
                    ...
                    F.expr('approx_most_frequent(payment_method, 1)[0][0]').alias('payment_method'),
                    F.expr('approx_most_frequent(store_id, 1)[0][0]').alias('store_id'),
                    F.expr('approx_most_frequent(register_id, 1)[0][0]').alias('register_id')
                )
