...
def create_spark_session():
    ...
    """
    spark = (
        SparkSession.builder.appName("TheGoods-Streaming-Processor")
        .config(
            "spark.jars.packages",
            "org.apache.hadoop:hadoop-aws:3.3.1,org.postgresql:postgresql:42.3.1,org.apache.spark:spark-sql-kafka-0-10_2.12:3.3.0",
        )
        .config(
...
def process_batch(df, epoch_id):
    ...
    try:
        ...
        logger.info(
            f"Accumulated {len(new_records)} records, total: {len(accumulated_records)}"
        )
        ...
        sales_fact_df = (
            sales_fact_df.withColumn(
                "sale_date", to_timestamp(col("sale_time")).cast("date")
            )
...
def cleanup_resources():
    ...
    if accumulated_records:
        logger.info(
            f"Saving {len(accumulated_records)} remaining records before shutdown"
        )
        bucket_name = minio_config.MINIO_CONFIG["bucket"]
